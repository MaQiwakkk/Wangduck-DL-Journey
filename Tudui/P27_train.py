...
if __name__ == '__main__':
    ...
    # 加载数据集
    # 训练集：打乱顺序，不多进程，不锁页内存
    train_dataloader = DataLoader(train_data, batch_size=64, shuffle=True, num_workers=0, pin_memory=False)
    # 测试集：不需要打乱，不多进程，不锁页内存
    test_dataloader = DataLoader(test_data, batch_size=64, shuffle=False, num_workers=0, pin_memory=False)
